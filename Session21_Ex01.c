...
void searchStudent(struct Students student[], int *sizePtr) {
    fflush(stdin);
    char *id;
    int found = 0;
    id = (char *)calloc(10, sizeof(char));
    printf("\nNhap ID sinh vien muon tim: ");
    fgets(id, 10, stdin);
    id[strcspn(id, "\n")] = '\0';
    for (int i = 0; i < *sizePtr; i++) {
        if (strstr(student[i].id, id) != NULL) {
...
void sortStudents(struct Students student[], int *sizePtr) {
    for (int i = 0; i < *sizePtr - 1; i++) {
        for (int j = i + 1; j < *sizePtr; j++) {
    ...
    printf("\nDanh sach sinh vien da duoc sap xep\n\n");
}
...
int main() {
    ...
    do {
        ...
        switch(choice) {
            ...
            case 5:
                searchStudent(student, sizePtr);
                break;
                
            case 6:
                sortStudents(student, sizePtr);
                saveToFile(student, sizePtr);
                break;
